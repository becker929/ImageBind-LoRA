...
from datasets.tuning4 import Tuning4Dataset
from datasets.image2audio import Image2AudioDataset
...
if __name__ == "__main__":
    ...
    if "tuning4" in args.datasets:
        train_datasets.append(
            Tuning4Dataset(
                root_dir=os.path.join(args.datasets_dir, "tuning4"),
                split="train",
                transform=ContrastiveTransformations(
                    contrast_transforms, n_views=2 if args.self_contrast else 1
                ),
    ...
    if set(args.datasets) - set(["dreambooth", "tuning4"]):
        for dataset in set(args.datasets) - set(["dreambooth", "tuning4"]):
            train_datasets.append(
                Image2AudioDataset(
                    data_dir=os.path.join(args.datasets_dir, dataset, "train"),
                    transform=ContrastiveTransformations(
                        contrast_transforms, n_views=2 if args.self_contrast else 1
                    ),
                )
            ...
            test_datasets.append(
                Image2AudioDataset(
                    data_dir=os.path.join(args.datasets_dir, dataset, "val"),
                    transform=ContrastiveTransformations(
                        contrast_transforms, n_views=2 if args.self_contrast else 1
                    ),
                )
